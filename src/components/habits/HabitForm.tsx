...
function HabitForm({ onSave, onCancel, existingHabit }: HabitFormProps) {
  ...
  const handleHabitForm = async (e: React.FormEvent) => {
    ...
    if (existingHabit) {
      ...
      saveHabits(existing.map((h) => (h.id === existingHabit.id ? updatedHabit : h)));
    } else {
  ...
  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 px-4">
      <div
        ...
        className="w-full max-w-md bg-zinc-900 p-6 sm:p-8 rounded-2xl border border-gray-800 shadow-lg"
      >
